...
function draw() {
    ...
    if ((c%n) > 2) {
	arrow(ox, oy, ox+len*2,  oy-len, color(0, 0, 255));
	strokeWeight(1); text("e₁", ox+len-5, oy+20);
    }
    if ((c%n) > 3) {
	arrow(ox, oy, ox+len, oy-len*3, color(255, 0, 0));
	strokeWeight(1); text("e₂", ox+10, oy-len);
    }
    if ((c%n) > 4) {
	let a=(3/5)*(mouseX-ox)-(1/5)*(mouseY-oy);
	arrow(ox, oy, ox+a*len*2, oy-a*len*2, color(100, 100, 200));
	strokeWeight(1); text("xe₁", mouseX-5, oy+20);
	stroke(color(180));line(mouseX, oy, mouseX, mouseY);
    }
    if ((c%n) > 5) {
	arrow(ox, oy, ox+(-1/5)*(mouseX-ox), oy+(2/5)*(mouseY-oy), color(200, 100, 100));
